...
func TestParseHours(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			desc:  "commas with /",
			input: "1,2/5",
			want:  []int{1, 2, 7, 12, 17, 22},
		},
		{
			desc:  "commas with / and -",
			input: "1,2-5/5",
			want:  []int{1, 2, 3, 4, 5, 10, 15, 20},
		},
	}
	for _, tC := range testCases {
		t.Run(tC.desc, func(t *testing.T) {
	...
	for _, tC := range testCases {
		t.Run(tC.desc, func(t *testing.T) {
			...
			if !reflect.DeepEqual(service.Minute, tC.want.Minute) {
				t.Errorf("parseMinute(%s) = %v, want %v", tC.input, service.Minute, tC.want.Minute)
			}
			if !reflect.DeepEqual(service.Hour, tC.want.Hour) {
				t.Errorf("parseHour(%s) = %v, want %v", tC.input, service.Hour, tC.want.Hour)
			}
			if !reflect.DeepEqual(service.DayOfMonth, tC.want.DayOfMonth) {
				t.Errorf("parseDayOfMonth(%s) = %v, want %v", tC.input, service.DayOfMonth, tC.want.DayOfMonth)
			}
			if !reflect.DeepEqual(service.Month, tC.want.Month) {
				t.Errorf("parseMonth(%s) = %v, want %v", tC.input, service.Month, tC.want.Month)
			}
			if !reflect.DeepEqual(service.DayOfWeek, tC.want.DayOfWeek) {
				t.Errorf("parseWeek(%s) = %v, want %v", tC.input, service.DayOfWeek, tC.want.DayOfWeek)
			}
			if !reflect.DeepEqual(service.Command, tC.want.Command) {
				t.Errorf("parseCommand(%s) = %v, want %v", tC.input, service.Command, tC.want.Command)
			}
		})
	}
}
